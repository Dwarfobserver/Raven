...
//-----------------------------------------------------------------------------
Goal_SafeTrip::
Goal_SafeTrip(Raven_Bot*          pBot,
	std::list<PathEdge> path) :Goal_Composite<Raven_Bot>(pBot, goal_safetrip),
	m_Path(path)
{
}
...
void Goal_SafeTrip::Activate()
{
	...
	{
		// Si c'est le dernier noeud, la destination , on straffe pas
		if (m_Path.empty()) {
		...
		// Sinon, on straffe
			...
			AddSubgoal(new Goal_DodgyWalk(m_pOwner, edge, m_Path.empty()));
		}
	}
