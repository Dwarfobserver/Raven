...
double GetMaxRoundsBotCanCarryForWeapon(int WeaponType)
{
  ...
  case type_slowering_gun:
	  ...
	  return script->GetDouble("SloweringGun_MaxRoundsCarried");
...
double Raven_Feature::TotalWeaponStrength(Raven_Bot* pBot)
{
  ...
  const double MaxRoundsSlowRay = GetMaxRoundsBotCanCarryForWeapon(type_slowering_gun);
  const double TotalRoundsCarryable = MaxRoundsForShotgun + MaxRoundsForRailgun + MaxRoundsForRocketLauncher + MaxRoundsSlowRay;
  ...
  double NumSlowRay    = (double)pBot->GetWeaponSys()->GetAmmoRemainingForWeapon(type_slowering_gun);
  ...
  return Tweaker + (1-Tweaker)*(NumSlugs + NumCartridges + NumRockets)/(MaxRoundsForShotgun + MaxRoundsForRailgun + MaxRoundsForRocketLauncher + MaxRoundsSlowRay);
}
